...
The policy does not depend on a privileged keeper, but production availability still benefits from independent automation that monitors `DrawRequested`, relays KMS proofs, and calls timeout cancellation. The active Sepolia deployment enforces this request and finalization policy; participant activation in the current source requires its own verified migration before use.
