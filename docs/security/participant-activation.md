...
ERC-7984 returns the actual transfer result as an encrypted value, including encrypted zero when a wallet lacks funds. CipherPool must not branch on that value synchronously or add every callback sender to its permanent draw array.
