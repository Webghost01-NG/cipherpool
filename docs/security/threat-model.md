...
Request and finalization are permissionless. Immutable prize and cadence parameters prevent a requester from choosing prize size or opening repeated locks, and the constructor requires at least one cancellation-length recovery window between requests. A keeper can only relay the readiness bit and KMS proof: `FHE.checkSignatures` binds them to the request's stored encrypted handles. A valid but ineligible snapshot releases the lock through `DrawSkipped`; invalid proofs leave the request intact. Deleting the request before winner evaluation and reverting when no request exists preserve single-use settlement and replay protection.
