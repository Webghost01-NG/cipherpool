...
6. On the active Sepolia runtime, any keeper submits `finalizeDraw(ready, proof)`, where the KMS proof reveals only whether positive weight and sufficient reserve both hold. A verified false result emits `DrawSkipped` and releases the lock without awarding a prize.
