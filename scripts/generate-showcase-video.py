...
NARRATION = [
    ...
    "The journey has four explicit steps. Deposit testnet c U S D C. Contribute or monitor the sponsor funded reserve. Participate in an encrypted weighted draw. Then withdraw c U S D C directly. Every transaction still requires wallet confirmation.",
    "The contracts bind encrypted credit to transferred assets and consume sponsor-funded prize reserves when awarding prizes. Sepolia does not claim generated yield. On chain signature verification and a cancellation timeout protect settlement.",
    "This is not a simulated transaction. A real one c U S D C deposit and direct withdrawal completed on Sepolia, followed by a real encrypted one c U S D C sponsor contribution. The transaction hashes and authorized K M S verification are linked in the repository.",
    "The project currently passes one hundred automated checks across Solidity contracts, backend services, client adapters, and frontend behavior. The frontend runs on Vercel, the backend on Render, and Postgre S Q L storage lets the indexer resume from its saved checkpoint after restart.",
